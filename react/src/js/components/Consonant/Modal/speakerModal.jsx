...
import prettyFormatDate from '../../Consonant/Helpers/prettyFormat';
...
const SpeakerModal = ({
    ...
    const format = (start, end) => (start
        ? prettyFormatDate(start, end, locale, i18nFormat)
        : '');
                            ...
                            <div style={{ margin: '12px 0' }}>
                                {sessions.map(s => (
                                    <div key={`${s.sessionId || s.sessionCode || s.sessionTitle}`} style={{ marginBottom: '10px' }}>
                                        ...
                                        <div>{format(s.sessionStartTime, s.sessionEndTime)}</div>
